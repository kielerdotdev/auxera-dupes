...
function PaymentButton({ itemId, price, disabled }) {
	...
	if (currentState.tooltip) {
		return <Tooltip label={currentState.tooltip}>{elements}</Tooltip>;
	}
					...
					<>
						...
						<MenuGroup title="Admin">
							<MenuItem
								onClick={async () => {
									const { data } = await engine.post(
										`/admin/items/toggleenabled/${itemId}`
									);
									toast({
										title: `${data?.disabled ? 'Disabled' : 'Enabled'} item`,
										status: 'success',
							...
							>
								{disabled?.type === 'forced' ? 'Aktiver' : 'Deaktiver'}
							</MenuItem>
						</MenuGroup>
					</>
				) : null}
...
export default function DetailsPage({ match }) {
	...
	const currentVersion = useLatestItemVersion(data);
	...
	return (
		...
		>
			<Box background=/*'gray.900'*/ {backgroundColor} p="8px">
				...
				<Box mt="8px">
					...
					<Flex>
						...
						<Text fontWeight="bold" ml="8px">
							{data?.author ? (
								<Link to={`/user/${data.author._id}`}>{data.author.name}</Link>
							) : (
								<Text fontWeight="bold" ml="8px">
									{'loading'}
								</Text>
